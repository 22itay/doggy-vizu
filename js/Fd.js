...
function click(d) {
    if (!d3.event.defaultPrevented) {
      ...
      function update() {
          console.log("update")
        var nodes2 = flatten(JSD),
            links = d3.layout.tree().links(nodes2);
        ...
        // Update the nodes…
        node = node.data(nodes2, function(d) { return d.id; }).style("fill", color);
        ...
        node.enter().append("circle")
            ...
            .attr("cy", function(d) { return d.y; })
            .attr("r", function(d) { return Math.sqrt(d.size) /15 || 4.5; })
            .style("fill", color)
            .on("click", click)
            .call(force.drag);
      }
